...
namespace raven
{
    ...
    {
        // clear any previous vertices and edges, preserve directed or undirected
        bool preserve = myfDirected;
        clear();
        myfDirected = preserve;
    ...
    vVertex_t cGraph::adjacentIn(vertex_t v)
    {
        ...
        for (int w = 0; w < vVertexName.size(); w++)
        {
            for (auto ei : vOutEdges[w])
            {
                if (vEdgeDst[ei] == v)
                {
                    ret.push_back(w);
